    ...
    return (
      <main className="flex-1 flex items-center justify-center p-6">
        ...
        <Card className="w-full max-w-md text-center">
          <CardHeader>
            <CardTitle>Scan de « {scan.brand_name} » en cours…</CardTitle>
            <CardDescription>
              On pose en direct 10 vraies questions d&apos;achat aux IA et on regarde si ta marque est citée.
              Environ une minute.
            </CardDescription>
          </CardHeader>
          <CardContent>
  ...
  return (
    <main className="flex-1 p-6 max-w-3xl mx-auto w-full grid gap-6">
      <header className="text-center grid gap-2">
        <p className="text-sm text-muted-foreground">Résultat du scan IA</p>
          ...
          <CardHeader>
            ...
            {teaser.shock ? (
              <>
                ...
                <CardDescription>
                  est cité {teaser.shock.competitorCount} fois par les IA sur ces mêmes questions
                  {teaser.shock.targetCount === 0 ? " — et toi zéro." : "."}
                </CardDescription>
              </>
            ) : (
      ...
      {!unlocked ? (
        <Card className="border-2">
          <CardHeader>
            <CardTitle>Débloque le rapport complet</CardTitle>
            <CardDescription>
              Le détail question par question : qui est cité, à quelle position, sur quel modèle —
              et les marques qui prennent ta place.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form action={submitLead} className="flex flex-col sm:flex-row gap-2">
              <input type="hidden" name="scanId" value={scan.id} />
              <Input type="email" name="email" required placeholder="ton@email.com" className="flex-1" />
              <Button type="submit">Voir le rapport complet</Button>
            </form>
          </CardContent>
        </Card>
      ) : (
        <>
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Qui les IA citent sur tes questions</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="grid gap-2">
                {teaser.topBrands.map((b) => (
                  <li key={b.name} className="flex justify-between text-sm">
                    <span>
                      {b.name} {b.isTarget && <Badge>toi</Badge>}
                    </span>
          ...
          <Card className="text-center">
            <CardHeader>
              <CardTitle className="text-base">Suis ton score chaque semaine, face à tes concurrents</CardTitle>
              <CardDescription>
                Ce scan est une photo. Mentio suit ta visibilité IA dans le temps — gratuitement.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button asChild>
                <Link href="/signup">Créer mon suivi gratuit</Link>
              </Button>
            </CardContent>
          </Card>
        </>
      ...
      <p className="text-xs text-muted-foreground text-center">
        Mesure basée sur les API des modèles avec recherche web activée — un très bon proxy des réponses
        grand public, pas une copie exacte.
      </p>
    </main>
  );
}
